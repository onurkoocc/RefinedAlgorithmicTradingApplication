...
class FeatureEngineer:
    ...
    def _process_data_in_chunks(self, df_full: pd.DataFrame, chunk_size: int) -> pd.DataFrame:
        ...
        results = []
        overlap = max(200, self.config.get("model", "sequence_length", 60) * 2)
        ...
        for i in range(0, len(df_full), chunk_size - overlap):
            ...
            try:
                ...
                if not processed_chunk.empty:
                    ...
                    slice_end = len(processed_chunk) - overlap if chunk_end < len(df_full) else len(processed_chunk)

                    if slice_start < slice_end:
                        results.append(processed_chunk.iloc[slice_start:slice_end])
                    elif i == 0 and slice_end > 0:
                        results.append(processed_chunk.iloc[:slice_end])
    ...
    def _add_advanced_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(df_adv) >= self.market_regime_util.lookback_period:
            regime_outputs = [self.market_regime_util.detect_regime(
                df_adv.iloc[max(0, i - self.market_regime_util.lookback_period):i])
                              for i in range(1, len(df_adv) + 1)]
            ...
            df_adv['volatility_regime'] = [r['metrics'].get('normalized_atr', 0.01) / 0.03 for r in regime_outputs]
        ...
        if 'taker_buy_base_asset_volume' in df_adv.columns and 'volume' in df_adv.columns:
            ...
            df_adv['cumulative_delta'] = (buy_vol - sell_vol).rolling(window=20, min_periods=5).sum()
        ...
        if 'close' in df_adv.columns and 'volume' in df_adv.columns:
            ...
            df_adv['vwap'] = (typical_price * df_adv['volume']).rolling(window=rolling_vwap_window,
                                                                        min_periods=5).sum() / \
                             df_adv['volume'].rolling(window=rolling_vwap_window, min_periods=5).sum()
        ...
        if 'atr_14' in df_adv.columns and 'close' in df_adv.columns:
            ...
            df_adv['vol_norm_close_change'] = df_adv['close'].pct_change(5).fillna(0) / (atr_norm + 1e-9)
            df_adv['vol_norm_momentum'] = df_adv['close'].pct_change(10).fillna(0) / (atr_norm + 1e-9)
    ...
    def _clean_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df_clean.columns:
            ...
            elif 'volume' in col or 'turnover' in col or 'delta' in col:
                df_clean[col] = df_clean[col].fillna(0)
            elif 'rsi' in col:
                df_clean[col] = df_clean[col].fillna(50)
            elif 'macd' in col or 'adx' in col or 'di' in col or 'trend_strength' in col:
                df_clean[col] = df_clean[col].fillna(df_clean[col].median()).fillna(0)
            elif 'bb_' in col or 'atr' in col or 'volatility' in col:
                df_clean[col] = df_clean[col].fillna(df_clean[col].median()).fillna(df_clean[col].mean()).fillna(0)
            else:
                df_clean[col] = df_clean[col].fillna(df_clean[col].median()).fillna(0)
