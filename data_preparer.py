...
class DataPreparer:
    def __init__(self, config):
        ...
        self.fallback_indicators = {
            "rsi_14": 50, "cci_20": 0, "willr_14": -50, "macd_histogram_12_26_9": 0,
            "bb_percent_b": 0.5, "market_regime": 0, "volatility_regime": 0.5,
            "taker_buy_ratio": 0.5, "mfi": 50, "hour_sin": 0, "hour_cos": 1,
        ...
        try:
            if os.path.exists(self.scaler_path):
                with open(self.scaler_path, 'rb') as f:
                    ...
                    if isinstance(self.scaler, dict): self.scaler_dict = self.scaler
    ...
    def _save_scaler_and_features(self):
        try:
            ...
            if self.feature_names is not None:
                with open(self.feature_list_path, 'w') as f: json.dump(self.feature_names, f)
        except Exception as e:
    ...
    def _calculate_feature_importance(self, df: pd.DataFrame, target_returns: np.ndarray) -> None:
        try:
            if len(df) < 200 or len(target_returns) < 200: return
            ...
            self.feature_importance = {feature: float(importance[i]) for i, feature in enumerate(df.columns)}
        except Exception as e:
    ...
    def _get_available_features(self, df: pd.DataFrame) -> List[str]:
        ...
        if self.use_only_essential_features:
            return [f for f in self.essential_features if f in df_columns or
                    f'm30_{f}' in df_columns and f not in available_features][:self.max_features]
        ...
        scored_features = [(feature, self.feature_importance.get(feature, 0.0)) for feature in available_features]
    ...
    def prepare_test_data(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, pd.DataFrame, np.ndarray]:
        ...
        required_features = self.feature_names if self.feature_names else self.essential_features
    ...
    def _handle_outliers(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df_clean.select_dtypes(include=np.number).columns:
            if col.startswith('actual_'): continue

            if col in ['open', 'high', 'low', 'close', 'volume']:
                q1, q3 = df_clean[col].quantile(0.001), df_clean[col].quantile(0.999)
        ...
        try:
            ...
            for i, train_feature in enumerate(self.feature_names):
                if train_feature in test_feature_names:
                    test_to_train_idx[i] = test_feature_names.index(train_feature)
        ...
        try:
            ...
            if cols_with_nans and len(df_clean) > 200:
                ...
                for col in cols_with_nans:
                    correlated_features = correlation_matrix[col][(correlation_matrix[col] > 0.7) &
                                                                  (correlation_matrix[col] < 1.0)].index.tolist()
                    if correlated_features:
                        ...
                        if mask.sum() > 100:
                            ...
                            model.fit(X, y)
                            df_clean.loc[nan_mask, col] = model.predict(X_pred)
        except Exception as e:
